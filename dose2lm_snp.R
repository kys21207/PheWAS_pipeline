...
#phenos.name="/GWD/appbase/projects/RD-TSci-UKB/PheWAS/phenotype500kNew/phenotypes_QT.txt"
#genos.name="/GWD/appbase/projects/RD-TSci-UKB/PheWAS_output_storage/Ioanna_snps_OA_201802091439/genotype"
#covar.name="/GWD/appbase/projects/RD-TSci-UKB/PheWAS/phenotype500kNew/covars_spe.txt"
#output.path="/GWD/appbase/projects/RD-TSci-UKB/PheWAS_output_storage/Ioanna_snps_OA_201802091439/snps_assoc"
...
covars <- covars[,1:3]
if (ncol(covars) > 0) {
  m0 <- lm(pheno1 ~ covars,na.action=na.omit)
} else {
  m0 <- lm(pheno1 ~ 1,na.action=na.omit)
}
...
if (ncol(covar_pcs) > 0) { # keep if statement outside vapply
  assoc <- as.data.frame(t(vapply(1:nrow(info), function(idx) {
    return(tryCatch({
      ...
      m1 <- suppressWarnings(lm(pheno1 ~ geno1 + covar_pcs,na.action=na.omit))
      beta <- coef(m1)["geno1"]
...
if(check == 0){
   write.table(cbind(info[sel, -2, drop = FALSE], info2[sel, , drop = FALSE], assoc[sel, , drop = FALSE]),
            file = paste(output.path,"/snp.linear",sep=""),
            row.names = FALSE, quote = FALSE)
   check <- 1
}else{
   write.table(cbind(info[sel, -2, drop = FALSE], info2[sel, , drop = FALSE], assoc[sel, , drop = FALSE]),
            file = paste(output.path,"/snp.linear",sep=""),col.names=FALSE,
