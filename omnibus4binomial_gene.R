...
#phenos.name<-"/GWD/appbase/projects/RD-TSci-UKB/PheWAS/phenotype500kNew/Sub_BI_pheno_1.txt"
#covars.name<-"/GWD/appbase/projects/RD-TSci-UKB/PheWAS/phenotype500kNew/covars.txt"
...
if (ncol(covars) > 0) { # keep if statement outside vapply
  assoc <- as.data.frame(t(vapply(1:nrow(info.genes), function(idx) {
    return(tryCatch({
      ...
      c(n, df1, beta, se, lrt1, lrt2)
    },
                    error = function(e) return(c(NA, NA, NA, NA, NA, NA))))},
                                  c(N = 0, DF = 0, BETA = 0., SE = 0., LRT = 0., LRTavg = 0.))))
}
...
sig.assoc<-assoc[assoc$PLRT <= sig.pvalue,]
