...
prepare_dta.forestPlot_QT <- function(dta,snp.name,p_cut,num_cut){

x=subset(dta, riskfactor %in% snp.name  & P < p_cut & P != 0)
if(nrow(x) > 0){
x=x[!duplicated(x),]
x=x[order(x$P),]
if(nrow(x) < num_cut) {
...
title <-paste("Plot of ",x$riskfactor[1],sep="")
...
prepare_dta.forestPlot_BI <- function(dta,snp.name,p_cut,num_cut){
...
if(!output.arg){
  for(i in output.files){
     ...
     dta <- subset(dta,!(Category %in% c("Brain imaging","Procedures")))
           score.dta <- subset(dta, TYPE2 %in% "SCORE" & !is.na(P))
           if(nrow(score.dta) != 0){
           sig.fdr<- 0.05
		   ...
		   p_cut<-0.05
		   num_cut <- 20
           SCORE.names <- unique(score.dta$riskfactor[!is.na(score.dta$riskfactor)]) 
           # generate data for plots for gene x SNP combinations
           for(j in SCORE.names){
	     sub.score.dta <- subset(score.dta, riskfactor %in% j)
             sub.score.dta$FDR <- p.adjust(sub.score.dta$P,method="BH")
             sig.p.snp <- 0.05/nrow(sub.score.dta)
	    for(k in types){
             sub.score.dta.type <- subset(sub.score.dta, TYPE1 %in% k)			
	     if(k == "QTL"){
   	       df<-prepare_dta.forestPlot_QT(sub.score.dta.type,as.character(j),p_cut,num_cut)
                if(!is.null(df)){ 
                 pdf(file=paste(output.path,"/plots/",gsub("/","_",as.character(j)),".FPQ.pdf",sep=""), height=8.5, width= 11)
                 plot<-forestPlot_QT(df)
                 print(plot)
                 dev.off()
                 }
             }else if(k == "Binary"){
               df<-prepare_dta.forestPlot_BI(sub.score.dta.type,as.character(j),p_cut,num_cut)
                if(!is.null(df)){ 
                 pdf(file=paste(output.path,"/plots/",gsub("/","_",as.character(j)),".FPB.pdf",sep=""), height=8.5, width= 11)
	     ...
	     data.plot <- prepare_dta.plot(sub.score.dta,cols,cutoff.OR,cutoff.qtl)
             data.plot[mapply(is.infinite, data.plot)] <- 300

             v <- as.character(j)
             file.out <-paste(output.path,"/plots/",gsub("/","_",as.character(j)),".MP.pdf",sep="")
